...
printf 'Answer: %s\n\n' "The terminal should show a structured denial and no environment-variable collection data should be displayed."
...
printf '$ %s\n' "$WARPCTRL --output-format json drive list --type env-var-collection"
"$WARPCTRL" --output-format json drive list --type env-var-collection > "/workspace/warpctrl-validation/drive-auth-execution/validation-artifacts/warpctrl-v2/f61caf49/drive-auth-execution/logs/004__drive_list_env_var_collection_denied.stdout_stderr.log" 2>&1
status=$?
cat "/workspace/warpctrl-validation/drive-auth-execution/validation-artifacts/warpctrl-v2/f61caf49/drive-auth-execution/logs/004__drive_list_env_var_collection_denied.stdout_stderr.log"
printf '\nexit_code=%s\n' "$status"
printf '%s\n' "$status" > "/workspace/warpctrl-validation/drive-auth-execution/validation-artifacts/warpctrl-v2/f61caf49/drive-auth-execution/logs/004__drive_list_env_var_collection_denied.exit_code"
touch "/workspace/warpctrl-validation/drive-auth-execution/validation-artifacts/warpctrl-v2/f61caf49/drive-auth-execution/logs/004__drive_list_env_var_collection_denied.done"
sleep 300
