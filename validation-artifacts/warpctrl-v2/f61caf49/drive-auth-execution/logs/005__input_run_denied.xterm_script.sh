...
printf '%s\n\n' "Use a staggered terminal/UI composition with the target Warp app visible before running the command; this command would visibly print text if it were allowed."
printf 'Answer: %s\n\n' "The terminal should show denial before execution, and the visible Warp app should not show a new terminal command or output containing warpctrl-validation."
...
"$WARPCTRL" --output-format json input run "printf warpctrl-validation" > "/workspace/warpctrl-validation/drive-auth-execution/validation-artifacts/warpctrl-v2/f61caf49/drive-auth-execution/logs/005__input_run_denied.stdout_stderr.log" 2>&1
status=$?
cat "/workspace/warpctrl-validation/drive-auth-execution/validation-artifacts/warpctrl-v2/f61caf49/drive-auth-execution/logs/005__input_run_denied.stdout_stderr.log"
printf '\nexit_code=%s\n' "$status"
printf '%s\n' "$status" > "/workspace/warpctrl-validation/drive-auth-execution/validation-artifacts/warpctrl-v2/f61caf49/drive-auth-execution/logs/005__input_run_denied.exit_code"
touch "/workspace/warpctrl-validation/drive-auth-execution/validation-artifacts/warpctrl-v2/f61caf49/drive-auth-execution/logs/005__input_run_denied.done"
sleep 300
