...
use crate::code_review::{CodeReviewTelemetryEvent, is_file_autogenerated};
...
use crate::util::git::{
    Commit, detect_current_branch, detect_main_branch, get_unpushed_commits,
...
/// Maximum size of a git object that we will read into memory as the base
/// content for diff comparison. Files larger than this are skipped to avoid
/// multi-GB heap growth when repositories have many large or generated files.
/// Matches the limit used by `num_lines_in_file_if_non_binary`.
const MAX_CONTENT_AT_HEAD_SIZE: u64 = 20 * 1000 * 1000; // 20MB
...
impl LocalDiffStateModel {
    ...
    /// Returns `true` if the given git object (e.g. `HEAD:src/foo.rs`) exceeds
    /// [`MAX_CONTENT_AT_HEAD_SIZE`] bytes, or if its size cannot be determined.
    ///
    /// Uses `git cat-file -s <object>` which is a fast metadata-only query that
    /// does not read the object data into memory.
    async fn is_git_object_too_large(repo_path: &Path, object: &str) -> bool {
        match run_git_command(repo_path, &["cat-file", "-s", object]).await {
            Ok(output) => output
                .trim()
                .parse::<u64>()
                .map(|size| size > MAX_CONTENT_AT_HEAD_SIZE)
                .unwrap_or(false),
            // If the object doesn't exist or can't be queried, let the
            // downstream `git show` handle the error naturally.
            Err(_) => false,
        }
    }
    ...
    ///
    /// Returns `None` for files exceeding `MAX_CONTENT_AT_HEAD_SIZE` to prevent
    /// large allocations from accumulating across all changed files simultaneously.
    async fn get_file_content_at_head(
        ...
        match status {
            ...
            _ => {
                // Guard against large files: reading the full content of every changed
                // file into memory simultaneously can cause multi-GB heap growth when
                // the repository has many large or generated files in its diff.
                if Self::is_git_object_too_large(repo_path, &format!("HEAD:{file_path}")).await {
                    log::debug!(
                        "[GIT OPERATION] local.rs get_file_content_at_head skipping HEAD:{file_path} (exceeds size limit)"
                    );
                    return None;
                }
                log::debug!(
    ...
    async fn get_file_content_at_commit(
        ...
        // Guard against large files: same rationale as get_file_content_at_head.
        let object = format!("{commit}:{file_path}");
        if Self::is_git_object_too_large(repo_path, &object).await {
            log::debug!(
                "[GIT OPERATION] local.rs get_file_content_at_commit skipping {object} (exceeds size limit)"
            );
            return None;
        }
        log::debug!(
        ...
        run_git_command(repo_path, &["show", &object]).await.ok()
    }
