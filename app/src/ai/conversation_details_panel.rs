...
impl ConversationDetailsPanel {
    ...
    fn render_environment_section(
        ...
        let docker_image = env_model.base_image.as_ref().map(ToString::to_string);
                ...
                } = &self.data.mode
                {
                    ...
                    if let Ok(server_id) = ServerId::try_from(env_id.as_str()) {
                        ...
                        if let Some(env) = CloudAmbientAgentEnvironment::get_by_id(&sync_id, ctx) {
                            if let Some(base_image) = &env.model().string_model.base_image {
                                let docker_image = base_image.to_string();
                                ctx.clipboard()
