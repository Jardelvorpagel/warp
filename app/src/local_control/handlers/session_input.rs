    ...
    let (previous_tab_index, active_tab_index, tab_count) =
        workspace.update(ctx, |workspace, ctx| {
            ...
            let workspace_action = match action {
                ActionKind::SessionPrevious => WorkspaceAction::CyclePrevSession,
                ActionKind::SessionNext => WorkspaceAction::CycleNextSession,
                _ => {
                    return (
                        previous_tab_index,
...
pub(crate) fn reopen_session(
    ...
    validate_session_cycle_target(ActionKind::SessionReopenClosed, target)?;
    ...
    let workspace = ctx
        ...
        .ok_or_else(|| {
            ControlError::new(
                ErrorCode::MissingTarget,
                "session.reopen_closed requires a workspace in the target window",
            )
        })?;
    workspace.update(ctx, |workspace, ctx| {
    ...
    Ok(json!({
        "action": ActionKind::SessionReopenClosed.as_str(),
        "handled": true,
        "window_id": window_id.to_string(),
    }))
}
