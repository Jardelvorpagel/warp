...
pub(crate) fn instance_inspect(instance_id: &Option<InstanceId>) -> serde_json::Value {
    json!({
...
pub(crate) fn window_inspect(
    ...
    let window_ids = select_window_ids(target, true, ActionKind::WindowInspect, ctx)?;
    let window_id = require_single_target(window_ids, ActionKind::WindowInspect, "window")?;
    let active_window = ctx.windows().active_window();
    let title = ctx
        .views_of_type::<Workspace>(window_id)
        .and_then(|workspaces| workspaces.into_iter().next())
...
pub(crate) fn tab_inspect(
    ...
    let entry = require_single_target(
        select_tab_entries(&target, ActionKind::TabInspect, ctx)?,
...
pub(crate) fn pane_inspect(
    ...
    let entry = require_single_target(
        select_pane_entries(&target, ActionKind::PaneInspect, ctx)?,
...
pub(crate) fn session_list(
    ...
    let concrete_session_filter = matches!(
        session_target,
        Some(SessionTarget::Id { .. } | SessionTarget::Index { .. })
    );
    let sessions = select_pane_entries(target, ActionKind::SessionList, ctx)?
        ...
        .filter_map(|entry| {
            ...
            let matches_session = match session_target {
                ...
                Some(SessionTarget::Index { index }) => entry.index as u32 == *index,
            };
        ...
        .collect::<Vec<_>>();
    if concrete_session_filter && sessions.is_empty() {
        return Err(ControlError::new(
            ErrorCode::StaleTarget,
            "session.list cannot resolve the requested session selector",
        ));
    }
    Ok(json!({
...
pub(crate) fn session_inspect(
    ...
    if target.session.is_none() {
        target.session = Some(SessionTarget::Active);
    }
    let data = session_list(&target, ctx)?;
    let sessions = data["sessions"].as_array().cloned().unwrap_or_default();
    let session = require_single_target(sessions, ActionKind::SessionInspect, "session")?;
    Ok(json!({
...
fn require_single_target<T>(
    ...
    match targets.len() {
        0 => Err(ControlError::new(
            ...
            format!("{} requires a matching {target_name}", action.as_str()),
        )),
        1 => Ok(targets.remove(0)),
        _ => Err(ControlError::new(
            ErrorCode::AmbiguousTarget,
            format!("{} resolved multiple {target_name}s", action.as_str()),
        )),
    }
}
...
fn select_tab_entries(
    ...
    let force_active_window = matches!(
        ...
        target.session,
        Some(SessionTarget::Active | SessionTarget::Index { .. })
    );
