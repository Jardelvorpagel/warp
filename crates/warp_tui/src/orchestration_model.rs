...
use warp::tui_export::{
    ...
    Harness, PreparedLocalOzChildLaunch, RenderableAIError, StartAgentExecutionMode,
    StartAgentRequest,
};
...
/// Events that require retained session views to refresh orchestration chrome.
...
impl TuiOrchestrationModel {
    ...
    pub(crate) fn register(ctx: &mut AppContext) -> ModelHandle<Self> {
        ...
        ctx.subscribe_to_model(&sessions, move |sessions, event, ctx| {
            match event {
            ...
            model_for_sessions.update(ctx, |model, ctx| {
                ...
                ctx.emit(TuiOrchestrationModelEvent::TabBarChanged);
                ctx.notify();
            });
        });
