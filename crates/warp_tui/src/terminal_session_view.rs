...
use crate::orchestration_model::{
    TuiOrchestrationModel, TuiOrchestrationModelEvent, TuiOrchestrationSnapshot,
};
...
pub(crate) fn init(app: &mut AppContext) {
    ...
    let cloud_context = id!("TuiTerminalSessionView") & id!(CLOUD_PLACEHOLDER_FLAG);
    app.register_editable_bindings([
        EditableBinding::new(
...
impl TuiTerminalSessionView {
    ...
    fn focus_current_owner(&mut self, ctx: &mut ViewContext<Self>) {
        if self.is_cloud_placeholder() {
            ctx.focus_self();
        } else if self.process_owns_input() {
    ...
    pub(crate) fn new(
        ...
        ctx.subscribe_to_view(&orchestration_tab_bar, |view, _, event, ctx| match event {
            TuiTabBarEvent::SelectTab(conversation_id) => {
                let Ok(conversation_id) = AIConversationId::try_from(conversation_id.clone())
                else {
                    return;
                };
        ...
        if ctx.has_singleton_model::<TuiOrchestrationModel>() {
            ctx.subscribe_to_model(
                ...
                |view, _, event, ctx| match event {
                    TuiOrchestrationModelEvent::Changed => {
    ...
    /// The conversation selected for prompts and tab highlighting.
        ...
        let tabs = snapshot
            ...
            .map(|tab| {
                let identity = palette
                    ...
                    .cloned()
                    .unwrap_or_default();
                let status_glyph = conversation_status_glyph(&tab.status);
                let status_style = conversation_status_glyph_style(&tab.status, builder);
...
impl TuiView for TuiTerminalSessionView {
    ...
    fn keymap_context(&self, ctx: &AppContext) -> keymap::Context {
        ...
        if self.orchestration_tabs_focused && !self.process_owns_input() {
