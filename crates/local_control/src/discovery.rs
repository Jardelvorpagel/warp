...
impl InstanceRecord {
    pub fn for_current_process(
        ...
        let credential_broker = outside_warp_control_enabled.then(|| CredentialBrokerReference {
            endpoint: endpoint.clone(),
        });
        let endpoint = outside_warp_control_enabled.then_some(endpoint);
        let actions = if outside_warp_control_enabled {
            actions
        } else {
            Vec::new()
        };
    ...
    pub fn endpoint(&self) -> Result<&ControlEndpoint, ControlError> {
        self.endpoint.as_ref().ok_or_else(disabled_record_error)
    }

    pub fn credential_broker(&self) -> Result<&CredentialBrokerReference, ControlError> {
        self.credential_broker
            .as_ref()
            .ok_or_else(disabled_record_error)
    }
}
...
fn disabled_record_error() -> ControlError {
    ControlError::new(
        ErrorCode::LocalControlDisabled,
        "outside-Warp local control is disabled for this instance",
    )
}
