        ...
        while let Some(matches) = captures.next() {
            for cap in matches.0.captures {
                ...
                if let Some(color) = color {
                    ...
                    // (the parse callback does `bytes.seek(ByteOffset::from(byte_offset + 1))`).
                    // to_buffer_char_offset(ByteOffset(N)) counts chars strictly *before*
                    // ByteOffset(N), so to include the char AT ByteOffset(N+1) (= tree-sitter
                    // byte N) we must pass N+2.  Adding 2 to both start and end produces
                    // CharOffset ranges that exactly match the buffer's 1-indexed CharOffset
                    // space (BlockMarker at CharOffset(1), first text char at CharOffset(2)).
                    let char_start =
                        ByteOffset::from(insertion_range.start + 2).to_buffer_char_offset(buffer);
                    let char_end =
                        ByteOffset::from(insertion_range.end + 2).to_buffer_char_offset(buffer);
